...
def make_tree_map(input_df):
    ...
    fig.update_layout(
        title_text='', title_x=0.5,
        annotations=[dict(
            text='Esta gráfica de Tree map muestra la relacion en proporciones de los segmentos de mercado con las cantidades de ventas por Region',
            showarrow=False,
            x=0.5, y=-0.15, xref='paper', yref='paper')])
    return fig
...
def make_sun(input_df):
    ...
    fig.update_layout(
        title_text='', title_x=0.5,
        annotations=[
            dict(text='Gráfico de Tarta para la representación de la prioridad de envíos con respecto a su categoría',
                 showarrow=False,
                 x=0.5, y=-0.15, xref='paper', yref='paper')])
    return fig
...
def make_pie(input_df):
    ...
    fig.update_layout(
        title_text='', title_x=0.5,
        annotations=[
            dict(text='Gráfico de Tarta que representa las proporciones de envios y costos por envíos por Región',
                 showarrow=False,
                 x=0.5, y=-0.15, xref='paper', yref='paper')])
    return fig
